...
from config import CHEAP_PS, EXCELLENT_GROSS_MARGIN, EXCELLENT_PROFIT_MARGIN, EXCELLENT_ROE, REASONABLE_PS
...
def ps_status(ps: Optional[float], eps_growth: Optional[float], forward_growth: Optional[float]) -> str:
    ...
    if ps <= REASONABLE_PS:
        return "סביר"
    strong_growth = (eps_growth is not None and eps_growth > 30) or (forward_growth is not None and forward_growth > 30)
